...
router.post("/signup/complete", async (req, res) => {
  ...
  if (!validation.nameValidation(name)) {
    return res.status(400).json({ message: "Invalid name", error: "INVALID_NAME" });
  }
...
router.patch("/update", async (req, res) => {
  ...
  try {
    ...
    if (!validation.glucagonLocationValidation(glucagonLocation)) {
      return res.status(400).json({ message: "Invalid glucagon location", error: errorTypes.INVALID_GLUCAGON_LOCATION });
    }

    if (!validation.glucagonTypeValidation(glucagonType)) {
