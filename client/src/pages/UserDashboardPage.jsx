...
const UserDashboardPage = () => {
    ...
    const [thresholdValue, setThresholdValue] = useState(user.lowAlarm || 180);
    ...
    const handleDexcomSessionIdRefresh = async () => {
        await updateDexcomSessionId(user.phoneNumber, user.dexcomUser, user.dexcomPass);
    };
    ...
    const handleSaveThreshold = async () => {
        const result = await updateThresholdValue(user.phoneNumber, parseInt(thresholdValue));
        if (result) {
